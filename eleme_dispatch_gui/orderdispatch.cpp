...
void OrderDispatch::start_dispatch(FILE *p_out)
{
    this->p_out = p_out;
    while (1)
    {
        ...
        {
            std::unique_lock lck(mu_deque);
            for(auto &p:dequed_order)
            {
                add_order(p);
            }
            dequed_order.clear();
            ...
            {
                finished = true;
                emit revoke_result(this);
                break;
            }
            else if(ret == 2)
            {
                finished = true;
                emit bankrupt_result(this);
                break;
            }
        }
        emit refresh_result(this);
        std::this_thread::sleep_for(std::chrono::seconds(CELL_TIME));
    }
}

void OrderDispatch::add_order_to_deque(Order o)
{
    qDebug()<<"add_order_to_deque before";
    std::unique_lock<std::mutex> lck(mu_deque);
    o.id = global_id++;
    dequed_order.push_back(o);
    qDebug()<<"add_order_to_deque end";
}
